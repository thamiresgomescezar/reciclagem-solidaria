...
async function carregarLista(listaContainer, feedbackMsg, perfil) {
  ...
  try {
    ...
    // Caso não haja coletas atreladas
    if (!coletas || !Array.isArray(coletas) || coletas.length === 0) {
      const msgVaziaTitle = éCatador 
    ...
    coletas.forEach(coleta => {
      ...
      let acoesHtml = '';
      if (éCatador) {
        // Catador NÃO confirma retirada (apenas visualiza endereço e tira dúvidas com ofertante específico)
        if (stNome === 'agendado') {
          acoesHtml = `
        ...
        // Visão Cidadão / Admin (Cidadão confirma suas ofertas e Admin confirma qualquer uma)
        if (stNome === 'agendado') {
          acoesHtml = `
    ...
    // Ouvinte para os botões de abrir mapa em modal na própria página
    ...
    // Eventos dos botões de ação utilizando o Modal do Sistema
